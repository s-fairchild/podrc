#!/usr/bin/env bats
# Exercises home/bin/kubernetes-mcp-kubeconfig-secret directly, not via
...
setup() {
  ...
  SCRIPT="${REPO_ROOT}/home/bin/kubernetes-mcp-kubeconfig-secret"
...
#######################################
# Copies the fixture kubeconfig to $HOME/.kube/config.
# Globals:
#   HOME
#   FIXTURES_DIR
#######################################
...
@test "kubernetes-mcp-kubeconfig-secret: dies when ~/.kube/config does not exist" {
  run "${SCRIPT}"
  assert_failure
  [[ "${status}" -eq 1 ]]
...
@test "kubernetes-mcp-kubeconfig-secret: never invokes podman when the kubeconfig is missing" {
  run "${SCRIPT}"
  assert_failure
...
@test "kubernetes-mcp-kubeconfig-secret: creates the secret from ~/.kube/config with the default name" {
  write_fixture_kubeconfig
...
@test "kubernetes-mcp-kubeconfig-secret: honors a KUBECONFIG_PODMAN_SECRET_NAME override" {
  ...
  refute_output --partial "kubernetes-mcp-kubeconfig "
}

@test "kubernetes-mcp-kubeconfig-secret: always passes --replace so re-runs overwrite the secret" {
  write_fixture_kubeconfig
...
@test "kubernetes-mcp-kubeconfig-secret: prints a confirmation naming the secret and source path" {
  ...
  run "${SCRIPT}"
  assert_success
  assert_output --partial "kubernetes-mcp-kubeconfig"
  assert_output --partial "${HOME}/.kube/config"
}

@test "kubernetes-mcp-kubeconfig-secret: propagates a podman failure instead of swallowing it" {
  write_fixture_kubeconfig
