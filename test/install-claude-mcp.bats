...
@test "install-claude-mcp: skips mcp-github when mcp-github-stdio isn't installed" {
	run "${REPO_ROOT}/hack/install-claude-mcp.sh"
	assert_success
...
@test "install-claude-mcp: registers mcp-github once mcp-github-stdio is installed" {
	...
	run grep -q "mcp add mcp-github --scope user -- ${HOME}/.local/bin/mcp-github-stdio" "${CLAUDE_STUB_LOG}"
	assert_success
}
