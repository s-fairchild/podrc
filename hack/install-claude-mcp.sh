...
# 127.0.0.1:8081/mcp -- see kubernetes-mcp.container's PublishPort= and
...
# Arguments:
#   name: MCP server name to check
# Returns:
#   0 if name is already registered, non-zero otherwise.
#######################################
...
# installed home/bin/github-mcp-server-stdio, skipping if already
# registered or if that script hasn't been installed yet.
# Outputs:
#   Writes status to the log.
#######################################
register_mcp_github() {
